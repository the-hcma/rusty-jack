...
tail --follow=name --retry "$HOME/Library/Logs/rusty-jack.log"
```

`rusty-jack status` shows the log path and a **`log follow`** row with the recommended live tail command. It also surfaces daemon health in the Daemon block; when the supervisor is not running, **`error`** is printed (in red in a terminal) and `status` exits non-zero. The Activity block shows the latest activity poll when available (idle time, console user, last idle→active transition, and last wake event when `event_tap` is active). Set `RUSTY_JACK_LOG_LEVEL=debug` before starting the daemon to log every activity poll; transitions log at `info` by default.
