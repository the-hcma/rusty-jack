...
Daemon logs are written in a single rotated file (`~/Library/Logs/rusty-jack.log` by default). `rusty-jack status` shows the log path, a recommended `tail -F` command, daemon health (errors in red when the LaunchAgent is installed but not running; non-zero exit for those cases), and the latest activity poll when available.
