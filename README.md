...
Daemon logs are written in a single rotated file (`~/Library/Logs/rusty-jack.log` by default). `rusty-jack status` shows the log path, a recommended `tail --follow=name --retry` command, daemon health (errors in red when the supervisor is not running; non-zero exit), and the latest activity poll when available.
